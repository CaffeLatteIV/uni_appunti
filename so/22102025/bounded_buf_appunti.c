...
void *producer(void *arg) { // void *args è richiesto da pthread 
	while (1) {
		int value;
		...
		semaphore_P(empty);
		buffer[front] = value;
		front = (front + 1) % SIZE; // se supera size allora ricomincia (grazie al % SIZE)
		semaphore_V(full);
		printf("sent: %d\n",value);
	}
}
...
int main(int argc, char *argv[]) {
	...
	srandom(time(NULL));
	pthread_create(&prod_t, NULL, producer, NULL); // il secondo NULL è l'elemento in input che dicevo prima (void *arg)
